...
set KEEPSAKE_URL=https://raw.githubusercontent.com/localplayer12/Tweaker/main/Storage/Keepsake.txt
set LOCAL_KEEPSAKE=Storage\Keepsake.txt
set GITHUB_KEEPSAKE=Keepsake_temp.txt
set OUTPUT=Xero.bat
set XERO_URL=https://raw.githubusercontent.com/localplayer12/Tweaker/main/Xero.bat
set GITHUB_REPO_URL=https://github.com/localplayer12/Tweaker/archive/refs/heads/main.zip
set REPO_ZIP=Tweaker-main.zip

if not exist %LOCAL_KEEPSAKE% (
    echo Local Keepsake.txt not found, downloading the entire repository...
    curl -L -o %REPO_ZIP% %GITHUB_REPO_URL%
    echo Unzipping repository...
    tar -xf %REPO_ZIP%
    goto :end
)

curl -L -o %GITHUB_KEEPSAKE% %KEEPSAKE_URL%

if not exist %GITHUB_KEEPSAKE% (
    echo Failed to download Keepsake.txt from GitHub.
    goto :end
)

for /f "tokens=2 delims==" %%A in (%LOCAL_KEEPSAKE%) do set LOCAL_VERSION=%%A
for /f "tokens=2 delims==" %%A in (%GITHUB_KEEPSAKE%) do set GITHUB_VERSION=%%A

echo Local version: %LOCAL_VERSION%
echo GitHub version: %GITHUB_VERSION%

if not "%LOCAL_VERSION%"=="%GITHUB_VERSION%" (
    echo Local version is outdated. Downloading new Xero.bat...
    curl -L -o %OUTPUT% %XERO_URL%
    if exist %OUTPUT% (
